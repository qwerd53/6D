...
from Oryon.utils.data import nocs, shapenet, common, toyl,ycbv,lm
from Oryon.utils.misc import torch_sample_select, unique_matches
from Oryon.utils.augmentations import *
from torch import Tensor
from PIL import Image
from Oryon.utils import viz
from torch.nn.functional import interpolate


def safe_preprocess_item(item):
    """纭繚杈撳嚭寮犻噺鏄疌HW鏍煎紡鐨刦loat32绫诲瀷"""
    processed = {}
    for k, v in item.items():
        if k == 'rgb':
            # 纭繚numpy鏁扮粍鏄痜loat32绫诲瀷
            if isinstance(v, np.ndarray):
                if v.dtype != np.float32:
                    v = v.astype(np.float32) / 255.0  # 褰掍竴鍖栧埌[0,1]
                # 璁板綍鍥惧儚灏哄 (H, W)
                processed['hw_size'] = torch.tensor(v.shape[:2], dtype=torch.int32)
            # 杞崲涓簍orch tensor骞剁‘淇濇槸CHW鏍煎紡
            rgb_tensor = torch.from_numpy(v).float()
            if rgb_tensor.ndim == 3 and rgb_tensor.shape[-1] == 3:  # HWC鏍煎紡
                rgb_tensor = rgb_tensor.permute(2, 0, 1)  # 杞负CHW
            processed[k] = rgb_tensor
        else:
            if isinstance(v, np.ndarray):
                # 澶勭悊闈濺GB鐨凬umPy鏁扮粍锛岃浆鎹int16绫诲瀷
                if v.dtype == np.uint16:
                    v = v.astype(np.float32)  # 鎴栬浆鎹负int32: v = v.astype(np.int32)
...
class CollateWrapper:
    def __init__(self, img_size=(256, 256)):
        """
         CollateWrapper锛堢畝鍖栫増锛?
        - 鏀寔 anchor/query 鍥惧儚銆乧amera銆乸ose銆乸rompt銆乮nstance_id
        - 鏀寔 mask 瀛楁
        """
        self.img_size = img_size

    def __call__(self, data: Sequence[Tuple]) -> dict:
        # Anchor 鍜?Query 鍚勮嚜鐨勬墍鏈夊瓧娈?
        ...
        for item_a, item_q, prompt, pose, obj_id, (inst_id_a, inst_id_q) in data:
            ...
            if mask_a is not None:
                ...
                masks_a.append(torch.zeros_like(img_a[0]))  # fallback 绌?mask
            ...
            if mask_q is not None:
                ...
                masks_q.append(torch.zeros_like(img_q[0]))  # fallback 绌?mask
...
class Shapenet6DDataset(Dataset):
    ...
    def __getitem__(self, index: int, i=0) -> Tuple:
        ...
        # prompt = self.get_item_prompt(item_a)[:1]  # shape 淇濇寔涓?[1, C, H, W]
...
class NOCSDataset(Dataset):
    ...
    def __init__(self, args: DictConfig, eval: bool = False):
        ...
        for instance in instances:
            ...
            # print(list(annots.keys())[:10])  # 鎵撳嵃鍓?0涓猭ey鐪嬬湅鏍煎紡

            if cat_id_a in self.obj_ids:
                pose_annot_id = '_'.join(
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        # 妫€鏌GB鍥惧儚褰㈢姸
        # print(f"Item A RGB shape: {item_a['rgb'].shape}")  # 搴旇鏄?(H,W,3)
        # print(f"Item Q RGB shape: {item_q['rgb'].shape}")  # 搴旇鏄?(H,W,3)
...
class TOYLDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        # print("item_a_pose:",item_a['metadata']['poses'][0])
        # print("item_q_pose:", item_q['metadata']['poses'][0])
        # print("pose:", pose)
...
class YCBVDataset(Dataset):
    ...
    def __init__(self, args: DictConfig, eval: bool = False):
        ...
        # }
        #NEW ONE
        #self.K = np.asarray([[1077.836, 0.0, 323.7872], [0.0, 1078.189, 279.6921], [0.0, 0.0, 1.0]])
        self.K = np.asarray([[ 1066.778, 0.0, 312.9869], [ 0.0, 1067.487, 241.3109], [0.0, 0.0, 1.0]])
        ...
        self.corrs = list()
        self.local_root = join(self.root, self.name)
        self.part_data = ycbv.get_part_data(self.local_root)
        self.obj_names = ycbv.get_obj_names(self.local_root)

        self.path_split = os.path.join(self.local_root, 'fixed_split', self.split)

        self.obj_models, self.obj_diams, self.obj_symms = ycbv.get_obj_data(join(self.local_root))
    ...
    def get_item(self, scene_id: int, img_id: int, obj_id: int, mask: str = 'oracle') -> dict:
        ...
        Wrapper usable outside the dataset
        '''
        item = ycbv.get_item_data(self.local_root, scene_id, img_id, self.part_data, self.obj_names, int(obj_id))
        return item
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        mask_type = get_mask_type(self.mask_type, self.eval)
        item_a = ycbv.get_item_data(self.local_root, scene_a, img_a, self.part_data, self.obj_names, cls_id, mask_type)
        item_q = ycbv.get_item_data(self.local_root, scene_q, img_q, self.part_data, self.obj_names, cls_id, mask_type)
...
class LMDataset(Dataset):
    ...
    def __init__(self, args: DictConfig, eval: bool = False):
        ...
        #self.K = np.asarray([[1077.836, 0.0, 323.7872], [0.0, 1078.189, 279.6921], [0.0, 0.0, 1.0]])
        self.K = np.asarray([[ 572.4114, 0.0, 325.2611], [ 0.0,573.57043, 242.04899], [0.0, 0.0, 1.0]])
        # 1066.778, 0.0, 312.9869, 0.0, 1067.487, 241.3109, 0.0, 0.0, 1.0
        with open(join(self.root, self.name, 'templates.json')) as f:
            self.prompt_templates = json.load(f)
        ...
        self.corrs = list()
        self.local_root = join(self.root, self.name)
        self.part_data = lm.get_part_data(self.local_root)
        self.obj_names = lm.get_obj_names(self.local_root)

        self.path_split = os.path.join(self.local_root, 'fixed_split', self.split)

        self.obj_models, self.obj_diams, self.obj_symms = lm.get_obj_data(join(self.local_root))
    ...
    def get_item(self, scene_id: int, img_id: int, obj_id: int, mask: str = 'oracle') -> dict:
        ...
        Wrapper usable outside the dataset
        '''
        item = lm.get_item_data(self.local_root, scene_id, img_id, self.part_data, self.obj_names, int(obj_id))
        return item
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        mask_type = get_mask_type(self.mask_type, self.eval)
        item_a = lm.get_item_data(self.local_root, scene_a, img_a, self.part_data, self.obj_names, cls_id, mask_type)
        item_q = lm.get_item_data(self.local_root, scene_q, img_q, self.part_data, self.obj_names, cls_id, mask_type)
