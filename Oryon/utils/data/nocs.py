...
from os.path import join
from Oryon.utils.pcd import lift_pcd, get_diameter
from Oryon.bop_toolkit_lib.misc import get_symmetry_transformations
...
def get_item_metadata(root: str, scene_id: int, img_id: int, pose_annots: dict, obj_names:dict, obj_name:Optional[str] = None) -> dict:
    ...
    for pose in pose_annots[f'{scene_id}_{img_id}']:
        ...
        ## 灏?NumPy 鏁扮粍杞崲涓?PyTorch 寮犻噺锛屽苟鎸囧畾鏁版嵁绫诲瀷
    ...
    with open(join(root, 'split/real_test', f'scene_{scene_id}/{img_id:04d}_meta.txt')) as fm:
        ...
        #     for i, (meta_line, det_line) in enumerate(zip(fm.readlines(), fd.readlines())):
        #         # ------------------------- 淇鐐?1锛氳В鏋?meta_line -------------------------
        #         # 瑙ｆ瀽 meta_line 鑾峰彇绫诲埆淇℃伅
        #         meta_parts = meta_line.strip().split()
        #         if len(meta_parts) < 3:
        #             print(f"璀﹀憡锛歮eta 鏂囦欢 {meta_line} 鏍煎紡閿欒锛岃烦杩囪琛?)
        #             continue
        #         mask_id_meta, cls_id_meta, cur_obj_name = meta_parts[0], meta_parts[1], meta_parts[2]
        #
        #         # 杩囨护鍙繚鐣欐寚瀹氱被鍒殑鏁版嵁锛堝鏋?obj_name 涓嶄负绌猴級
        #         if obj_name is not None and cur_obj_name != obj_name:
        #             continue
        #
        #         # 濉厖 cls_names
        #         cls_name = obj_names.get(cur_obj_name, ["unknown_class"])[0]  # 浣跨敤榛樿鍊奸伩鍏?KeyError
        #         cls_names.append(cls_name)
        #         # ------------------------------------------------------------------------
        #
        #         # 瑙ｆ瀽妫€娴嬫鏁版嵁锛堝師鏈夐€昏緫锛?
        #         parts = det_line.strip().split()
        #         if len(parts) < 5:
        #             H, W = 480, 640
        #             x, y, w, h = 0, 0, W, H
        #         else:
        #             _, x, y, w, h = parts[0], int(parts[1]), int(parts[2]), int(parts[3]), int(parts[4])
        #
        #         y_center = y + h // 2
        #         x_center = x + w // 2
        #         dets.append([y_center, x_center, h, w])
...
# def get_item_metadata(root: str, scene_id: int, img_id: int, pose_annots: dict, obj_names: dict,
#                       obj_name: Optional[str] = None) -> dict:
#     '''
#     Return NOCS dataset metadata given root, and ids of scene and image
#     '''
#
