...
def get_camera() -> np.ndarray:
    #camera = np.asarray([[1077.836, 0.0, 323.7872], [0.0, 1078.189, 279.6921], [0.0, 0.0, 1.0]])
    camera=np.asarray([[ 572.4114, 0.0, 325.2611], [ 0.0,573.57043, 242.04899], [0.0, 0.0, 1.0]])
    return camera
...
def get_item_metadata(root: str, scene_id: int, img_id: int, pose_annots: dict, cls_names_dict: dict,
                      cls_id: Optional[int] = None) -> dict:
...
def get_item_data(root: str, scene_id: int, img_id: int, pose_annots: dict, cls_names: dict,
                  cls_id: Optional[int] = None, mask_type: Optional[str] = 'oracle', hf_depth: bool = False) -> dict:
    '''
