...
class MatcherLoss(nn.Module):
    def __init__(self, config):
        ...
        matcher_cfg = config.get('matcher', config[legacy_matcher_key])
        self.loss_config = matcher_cfg['loss']
        self.match_type = matcher_cfg['match_coarse']['match_type']
        self.sparse_spvs = matcher_cfg['match_coarse']['sparse_spvs']
    ...
    #     loss = loss_c * self.loss_config['coarse_weight']
    #     loss_scalars.update({"loss_c": loss_c.clone().detach().cpu()})
    #     print(f"[FeatureMatcherLoss] coarse loss: {loss_c.item():.6f}")
    #
    #     # fine
    #     loss_f = self.compute_fine_loss(data['expec_f'], data['expec_f_gt'])
    #     if loss_f is not None:
    #         loss += loss_f * self.loss_config['fine_weight']
    #         loss_scalars.update({"loss_f": loss_f.clone().detach().cpu()})
    #         print(f"[FeatureMatcherLoss] fine loss: {loss_f.item():.6f}")
    #     else:
    #         assert self.training is False
    #         loss_scalars.update({'loss_f': torch.tensor(1.)})
    #         print(f"[FeatureMatcherLoss] fine loss: None (set to 1.0 for eval)")
    #
    #     print(f"[FeatureMatcherLoss] total loss: {loss.item():.6f}")
    #
    #     loss_scalars.update({'loss': loss.clone().detach().cpu()})
    #     data.update({"loss": loss, "loss_scalars": loss_scalars})
    #     return data
