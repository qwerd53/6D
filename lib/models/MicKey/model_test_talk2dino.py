...
from Talk2DINO.src.open_vocabulary_segmentation.models import build_model as build_t2dino_model
# =========================
#   MicKeyTrainingModel
# =========================
class MicKeyTrainingModel(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # ---------- Talk2DINO (用于掩码生成与过滤，按 demo.py 的方式) ----------
        t2dino_cfg_path = '/data/WDY/mickey-main/Talk2DINO/src/open_vocabulary_segmentation/configs/cityscapes/dinotext_cityscapes_vitb_mlp_infonce.yml'
        t2dino_device = 'cuda' if torch.cuda.is_available() else 'cpu'
        t2dino_cfg = OmegaConf.load(t2dino_cfg_path)
        self.t2dino_model = build_t2dino_model(t2dino_cfg.model)
        self.t2dino_model.to(t2dino_device).eval()
        self.t2dino_model.with_bg_clean = True
        # demo.py 默认类别
        #self.t2dino_texts = None
        #self.t2dino_with_bg = True  # 默认需要背景
        self.t2dino_apply_pamr = True
        #self.t2dino_fg_thresh = 0.5

        # with torch.no_grad():
        #     text_tokens = self.t2dino_model.build_dataset_class_tokens("sub_imagenet_template", self.t2dino_texts)
        #     self.t2dino_text_emb = self.t2dino_model.build_text_embedding(text_tokens)
    ...
    def forward_once(self, batch):
        ...
        # 使用 Talk2DINO 掩码（完全替代 Oryon 掩码与 IoU 计算）
        mask_loss_all = torch.tensor(0.0, device=device)
        mask_iou_mean = torch.tensor(0.0, device=device)
        mask0_loss = torch.tensor(0.0, device=device)
        mask1_loss = torch.tensor(0.0, device=device)

        # 3-5) 使用 Talk2DINO 生成用于过滤的二值掩码（不影响上面的 Oryon 损失）
        pred_mask0_list = []
        pred_mask1_list = []
        with torch.no_grad():
            for i in range(B):
                # 为该对图像构建共享的文本嵌入
                prompt_str = batch['prompt'][i][0]
                # if isinstance(prompt_str, bytes):
                #     prompt_str = prompt_str.decode('utf-8', errors='ignore')
                texts_i = prompt_str.replace('_', ' ').split(',')
                #texts_i = [t.strip() for t in texts_i if t.strip()]
                # if len(texts_i) == 0:
                #     texts_i = self.t2dino_texts
                #     print("error,text=None")
                print(" prompt_str:",prompt_str)
                print(" instance id:", batch['instance_id'][i])
                with torch.no_grad():
                    text_tokens_i = self.t2dino_model.build_dataset_class_tokens("sub_imagenet_template", texts_i)
                    text_emb_i = self.t2dino_model.build_text_embedding(text_tokens_i)



                # image0 掩码（demo.py 方式，默认含背景）
                img0 = batch['image0'][i:i+1].float()
                m0_probs, _ = self.t2dino_model.generate_masks(
                    img0, img_metas=None,
                    text_emb=text_emb_i,
                    classnames=texts_i,
                    apply_pamr=self.t2dino_apply_pamr
                )
                bg0 = torch.ones_like(m0_probs[:, :1]) * 0.55
                m0_probs = torch.cat([bg0, m0_probs], dim=1)
                m0_label = m0_probs.argmax(dim=1)
                m0_bin = (m0_label != 0).float().squeeze(0)
                if m0_bin.shape[-2:] != (H, W):
                    m0_bin = F.interpolate(m0_bin.unsqueeze(0).unsqueeze(0), size=(H, W), mode='nearest').squeeze(0).squeeze(0)
                pred_mask0_list.append(m0_bin)

                # image1 掩码（同一 prompt）
                img1 = batch['image1'][i:i+1].float()
                m1_probs, _ = self.t2dino_model.generate_masks(
                    img1, img_metas=None,
                    text_emb=text_emb_i,
                    classnames=texts_i,
                    apply_pamr=self.t2dino_apply_pamr
                )
                bg1 = torch.ones_like(m1_probs[:, :1]) * 0.55
                m1_probs = torch.cat([bg1, m1_probs], dim=1)
                m1_label = m1_probs.argmax(dim=1)
                m1_bin = (m1_label != 0).float().squeeze(0)
                if m1_bin.shape[-2:] != (H, W):
                    m1_bin = F.interpolate(m1_bin.unsqueeze(0).unsqueeze(0), size=(H, W), mode='nearest').squeeze(0).squeeze(0)
                pred_mask1_list.append(m1_bin)

                        # === 保存 Talk2DINO 掩码到 debug 目录 ===
                debug_dir = "debug/talk2dino_masks"
                os.makedirs(debug_dir, exist_ok=True)

                # 保存 image0 掩码
                mask0_np = (m0_bin.cpu().numpy() * 255).astype(np.uint8)
                plt.imsave(os.path.join(debug_dir, f"instance_id{batch['instance_id'][i]}_i{i}_img0_mask.png"), mask0_np, cmap='gray')

                # 保存 image1 掩码
                mask1_np = (m1_bin.cpu().numpy() * 255).astype(np.uint8)
                plt.imsave(os.path.join(debug_dir, f"instance_id{batch['instance_id'][i]}_i{i}_img1_mask.png"), mask1_np, cmap='gray')
                print("已保存一对")

        pred_mask0_bin = torch.stack(pred_mask0_list, dim=0).to(device)
        pred_mask1_bin = torch.stack(pred_mask1_list, dim=0).to(device)

        # 使用 GT 掩码计算 Talk2DINO 掩码的 IoU
        mask0_gt = batch['mask0_gt'].to(device).float()  # [B,H,W]
        mask1_gt = batch['mask1_gt'].to(device).float()  # [B,H,W]
        if mask0_gt.shape[-2:] != (H, W):
            mask0_gt = F.interpolate(mask0_gt.unsqueeze(1), size=(H, W), mode='nearest').squeeze(1)
        if mask1_gt.shape[-2:] != (H, W):
            mask1_gt = F.interpolate(mask1_gt.unsqueeze(1), size=(H, W), mode='nearest').squeeze(1)
        if mask0_gt.max() > 1.0:
            mask0_gt = mask0_gt / 255.0
        if mask1_gt.max() > 1.0:
            mask1_gt = mask1_gt / 255.0

        intersection0 = (pred_mask0_bin * mask0_gt).sum(dim=(1, 2))
        union0 = (pred_mask0_bin + mask0_gt - pred_mask0_bin * mask0_gt).sum(dim=(1, 2)) + 1e-6
        iou0 = (intersection0 / union0).mean()
        intersection1 = (pred_mask1_bin * mask1_gt).sum(dim=(1, 2))
        union1 = (pred_mask1_bin + mask1_gt - pred_mask1_bin * mask1_gt).sum(dim=(1, 2)) + 1e-6
        iou1 = (intersection1 / union1).mean()
        mask_iou_mean = (iou0 + iou1) / 2.0
        ...
        R_preds, t_preds = [], []
        for i in range(B):
            match_batch = {'image0': img0_gray[i:i + 1].float(), 'image1': img1_gray[i:i + 1].float()}
            with torch.no_grad():
                self.matcher.eval()
