...
class MicKeyTrainingModel(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        self.analysis_save_dir = "add_error_analysis"
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #add
        self._collect_add_errors_for_analysis(batch, out['R_pred'], out['t_pred'], batch_idx)
        # 计算MSSD, MSPD, VSD
    ...
    def _collect_add_errors_for_analysis(self, batch, R_pred, t_pred, batch_idx):
        """收集每个样本的ADD误差用于后续分析"""
        if not hasattr(self, 'obj_models'):
            return

        batch_size = len(batch['obj_id'])

        for i in range(batch_size):
            # 处理 obj_id
            obj_id_data = batch['obj_id'][i]
            obj_id = obj_id_data.item() if hasattr(obj_id_data, 'item') else int(obj_id_data)

            # 处理 scene_id
            scene_id_data = batch.get('scene_id', [0])[i] if 'scene_id' in batch else 0
            scene_id = scene_id_data.item() if hasattr(scene_id_data, 'item') else int(scene_id_data)

            # 处理 instance_id（batch 里已有）
            instance_id_data = batch['instance_id'][i]
            if hasattr(instance_id_data, 'item'):
                instance_id = instance_id_data.item()
            else:
                instance_id = str(instance_id_data)  # 避免是 tensor 时出错，转成 str 更安全
            ...
            # 保存到分析数据中
            self.add_error_analysis['add_errors'].append(add_error)
            self.add_error_analysis['instance_ids'].append(instance_id)
            self.add_error_analysis['obj_ids'].append(obj_id)
            self.add_error_analysis['scene_ids'].append(scene_id)
            self.add_error_analysis['pred_poses'].append(
                pred_pose.cpu().numpy() if pred_pose is not None else None
            )
            self.add_error_analysis['gt_poses'].append(
                gt_pose.cpu().numpy() if gt_pose is not None else None
            )
            self.add_error_analysis['sample_ids'].append(
    ...
    def on_validation_epoch_end(self):
        ...
        # 生成ADD误差分析图表
        if self.add_error_analysis['add_errors']:
            self._generate_add_error_analysis()

        # 清空输出
        self.validation_step_outputs.clear()
        # 清空ADD误差分析数据（可选，如果想累积多个epoch的数据就不要清空）
        # self.add_error_analysis = {key: [] for key in self.add_error_analysis}

    def _generate_add_error_analysis(self):
        """生成ADD误差分析图表"""
        os.makedirs(self.analysis_save_dir, exist_ok=True)

        # 转换为numpy数组
        add_errors = np.array(self.add_error_analysis['add_errors'])
        obj_ids = np.array(self.add_error_analysis['obj_ids'])
        instance_ids = np.array(self.add_error_analysis['instance_ids'])
        scene_ids = np.array(self.add_error_analysis['scene_ids'])

        # 过滤无效数据
        valid_mask = ~np.isnan(add_errors)
        valid_errors = add_errors[valid_mask]
        valid_obj_ids = obj_ids[valid_mask]
        valid_scene_ids = scene_ids[valid_mask]

        # 保存原始数据
        np.save(os.path.join(self.analysis_save_dir, "add_errors.npy"), add_errors)
        np.save(os.path.join(self.analysis_save_dir, "obj_ids.npy"), obj_ids)
        np.save(os.path.join(self.analysis_save_dir, "scene_ids.npy"), scene_ids)

        # 创建DataFrame
        df = pd.DataFrame({
            'sample_id': self.add_error_analysis['sample_ids'],
        ...
        df.to_csv(os.path.join(self.analysis_save_dir, "add_errors_full.csv"), index=False)
        ...
        # 1. ADD误差分布直方图
        plt.subplot(2, 2, 1)
        plt.hist(valid_errors, bins=50, alpha=0.7, color='blue', edgecolor='black')
        ...
        # 2. 按物体ID分组的平均ADD误差
        plt.subplot(2, 2, 2)
        obj_stats = pd.DataFrame({'obj_id': valid_obj_ids, 'error': valid_errors})
        obj_mean = obj_stats.groupby('obj_id')['error'].mean()
        obj_mean.plot(kind='bar', alpha=0.7, color='green')
        plt.xlabel('Object ID')
        plt.ylabel('Mean ADD Error (m)')
        plt.title('Mean ADD Error by Object')
        ...
        # 4. 按场景ID分组的平均ADD误差
        plt.subplot(2, 2, 4)
        scene_stats = pd.DataFrame({'scene_id': valid_scene_ids, 'error': valid_errors})
        scene_mean = scene_stats.groupby('scene_id')['error'].mean()
        scene_mean.plot(kind='bar', alpha=0.7, color='orange')
        plt.xlabel('Scene ID')
        plt.ylabel('Mean ADD Error (m)')
        plt.title('Mean ADD Error by Scene')
        plt.xticks(rotation=45)

        plt.tight_layout()
        plt.savefig(os.path.join(self.analysis_save_dir, "add_error_analysis.png"),
                    dpi=300, bbox_inches='tight')
        plt.close()

        # 打印统计信息
        print(f"\n=== ADD Error Analysis ===")
        print(f"Total samples: {len(add_errors)}")
        print(f"Valid samples: {len(valid_errors)}")
        print(f"Mean ADD error: {np.mean(valid_errors):.4f} m")
        print(f"Median ADD error: {np.median(valid_errors):.4f} m")
        print(f"Std ADD error: {np.std(valid_errors):.4f} m")
