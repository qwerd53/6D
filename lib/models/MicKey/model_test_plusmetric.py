...
# ✅ NEW: Helper function for calculating rotation and translation errors
def calc_rt_error(pred_R, pred_t, gt_R, gt_t):
    """
    Calculate rotation error (degrees) and translation error (cm)

    Args:
        pred_R, gt_R: [3, 3] numpy rotation matrices
        pred_t, gt_t: [3] or [3, 1] numpy translation vectors (unit: meter)

    Returns:
        r_err: rotation error in degrees
        t_err: translation error in cm
    """
    # 1. Translation Error (in cm)
    t_err = np.linalg.norm(pred_t.flatten() - gt_t.flatten()) * 100.0

    # 2. Rotation Error (in deg)
    # R_err = arccos((tr(R_pred.T @ R_gt) - 1) / 2)
    R_rel = pred_R.T @ gt_R
    tr = np.trace(R_rel)
    cos_theta = (tr - 1) / 2.0
    cos_theta = np.clip(cos_theta, -1.0, 1.0)
    r_err = np.arccos(cos_theta) * (180.0 / np.pi)

    return r_err, t_err


# ✅ NEW: 3D Volume IoU calculation (following 6-PACK/NOCS standard)
def compute_3d_iou(pred_R, pred_t, gt_R, gt_t, model_pts):
    """
    Calculate 3D Volume IoU between predicted and ground truth 3D bounding boxes.
    Following 6-PACK/NOCS evaluation standard.

    Args:
        pred_R: [3, 3] predicted rotation matrix
        pred_t: [3] predicted translation vector (meter)
        gt_R: [3, 3] ground truth rotation matrix
        gt_t: [3] ground truth translation vector (meter)
        model_pts: [N, 3] object model points in canonical space (meter)

    Returns:
        iou_3d: 3D Volume IoU value (0~1)
    """
    # Transform model points to world space
    pred_pts = (pred_R @ model_pts.T).T + pred_t.reshape(1, 3)  # [N, 3]
    gt_pts = (gt_R @ model_pts.T).T + gt_t.reshape(1, 3)  # [N, 3]

    # Compute 3D bounding boxes (axis-aligned in world space)
    pred_min = pred_pts.min(axis=0)  # [3]
    pred_max = pred_pts.max(axis=0)  # [3]
    gt_min = gt_pts.min(axis=0)  # [3]
    gt_max = gt_pts.max(axis=0)  # [3]
    ...
    inter_valid = np.all(inter_max > inter_min)
    ...
    iou_3d = inter_volume / (union_volume + 1e-8)
...
class MicKeyTrainingModel(pl.LightningModule):
    ...
    def mask_loss(self, pred_logits: torch.Tensor, gt: torch.Tensor):
        ...
        返回: loss, pred_mask(0/1), pred_logits, IoU_mean, IoU_per_sample
        """
        gt_shape = gt.shape[1:]
        ...
        with torch.no_grad():
            ...
            # ✅ MODIFIED: 计算 per-sample IoU，而不仅仅是 mean
            iou_per_sample = intersection / union  # [B]
            iou_mean = iou_per_sample.mean()

        # ✅ MODIFIED: 返回 iou_per_sample
        return loss, pred_mask, logits, iou_mean, iou_per_sample
    ...
    def forward_once(self, batch):
        ...
        if (self.useGTmask):
            ...
            # ✅ MODIFIED: 接收 iou_per_sample
            mask0_loss, _, _, mask0_iou, mask0_ious = self.mask_loss(mask0_gt, batch['mask0_gt'])
            mask1_loss, _, _, mask1_iou, mask1_ious = self.mask_loss(mask1_gt, batch['mask1_gt'])
        else:
            # ✅ MODIFIED: 接收 iou_per_sample (mask0_ious, mask1_ious)
            mask0_loss, _, _, mask0_iou, mask0_ious = self.mask_loss(pred_mask0_logits, mask0_gt)
            mask1_loss, _, _, mask1_iou, mask1_ious = self.mask_loss(pred_mask1_logits, mask1_gt)
        mask_loss_all = mask0_loss + mask1_loss
        mask_iou_mean = (mask0_iou + mask1_iou) / 2.

        # ✅ NEW: 合并两个视角的 IoU (取平均作为该样本的 IoU 表现)
        mask_ious_sample = (mask0_ious + mask1_ious) / 2.0
        ...
        return {
            ...
            'mask_ious_sample': mask_ious_sample,  # ✅ NEW: 传递出去用于 validation 计算 IoU25
    ...
    def on_validation_epoch_start(self):
        """Load object models before validation starts (needed for 3D IoU calculation)"""
        if not self._vsd_objects_loaded:
            try:
                self._load_vsd_objects()
                print(f"✅ Loaded {len(self.obj_models)} object models for 3D IoU calculation")
            except Exception as e:
                print(f"⚠️ Warning: Could not load object models for 3D IoU: {e}")
                print("   3D IoU will be set to 0 for all samples")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        r_errs = []  # 旋转误差 (deg)
        t_errs = []  # 平移误差 (cm)
        acc_5_5 = []  # 5度5cm 准确率 (bool)
        iou_25 = []  # 3D Volume IoU >= 0.25 (bool) - 6-PACK/NOCS standard

        for i in range(len(batch['obj_id'])):
            # 获取当前样本的位姿
            pred_R = out['R_pred'][i].detach().cpu().numpy()
            pred_t = out['t_pred'][i].detach().cpu().numpy().squeeze()  # [3] in meter

            gt_pose = batch['item_q_pose'][i].detach().cpu().numpy()
            gt_R = gt_pose[:3, :3]
            gt_t = gt_pose[:3, 3]  # [3] in meter

            # 计算 R_err, T_err, 5deg5cm
            r_err, t_err = calc_rt_error(pred_R, pred_t, gt_R, gt_t)
            r_errs.append(r_err)
            t_errs.append(t_err)

            # Check 5 deg, 5 cm
            is_5_5 = (r_err < 5.0) and (t_err < 5.0)
            acc_5_5.append(float(is_5_5))

            # ✅ NEW: Calculate 3D Volume IoU (6-PACK/NOCS standard)
            if hasattr(self, 'obj_models'):
                obj_id = batch['obj_id'][i]
                if obj_id in self.obj_models:
                    # Get model points in canonical space (mm -> meter)
                    model_pts = self.obj_models[obj_id]['pts'].astype(np.float32) / 1000.0  # [N, 3] in meter

                    # Calculate 3D Volume IoU
                    iou_3d = compute_3d_iou(pred_R, pred_t, gt_R, gt_t, model_pts)
                    iou_25.append(1.0 if iou_3d >= 0.25 else 0.0)
                else:
                    # Fallback: if object model not available, use 0
                    iou_25.append(0.0)
            else:
                # Fallback: if obj_models not loaded, use 0
                iou_25.append(0.0)

        # ✅ 只记录新增的4个指标
        logs = {
            'r_err': torch.tensor(np.mean(r_errs) if r_errs else 0.0),
            't_err': torch.tensor(np.mean(t_errs) if t_errs else 0.0),
            'acc_5_5': torch.tensor(np.mean(acc_5_5) if acc_5_5 else 0.0),
            'iou_25': torch.tensor(np.mean(iou_25) if iou_25 else 0.0),  # 3D Volume IoU
        }
    ...
    def on_validation_epoch_end(self):
        ...
        # ✅ 只聚合和显示新增的4个指标
        keys_to_log = ['r_err', 't_err', 'acc_5_5', 'iou_25']

        metrics = {}
        for k in keys_to_log:
            values = [x[k] for x in self.validation_step_outputs if k in x]
            if values:
                metrics[f'val/{k}'] = torch.stack(values).mean()

        # 记录所有指标，全部显示在进度条
        for name, value in metrics.items():
            self.log(name, value, on_epoch=True, sync_dist=self.multi_gpu, prog_bar=True)
