...
class CLIPEncoder(nn.Module):
    ...
    #     '''
    #     同时支持两种输入格式：
    #     1. 多模板: [["prompt1_temp1", "prompt1_temp2"], ["prompt2_temp1", ...]]
    #     2. 单prompt: ["prompt1", "prompt2"] (自动转换为单模板形式)
    #     '''
    #     # 类型统一化处理
    #     if isinstance(prompts[0], str):  # 检测到单prompt输入
    #         prompts = [[p] for p in prompts]  # 转换为[[p1], [p2],...]形式
    #
    #
    #
    #     # 原有处理逻辑保持不变
    #     # prompts = [prompt_list[1:] for prompt_list in prompts]
    #
    #     prompts = [prompt_list for prompt_list in prompts]  # for 0 templates prompt
    #
    #
    #     tokenized_text = torch.stack([self.tokenizer(prompt) for prompt in prompts])
    #     tokenized_text = tokenized_text.to(self.device)
    #
    #     if tokenized_text.ndim == 2:
    #         # 只有 [B, L]，没有 T 模板维度，手动加上 T=1
    #         tokenized_text = tokenized_text.unsqueeze(1)  # [B, 1, L]
    #
    #     B, T = tokenized_text.shape[:2]
    #
    #     # 后续处理无需修改
