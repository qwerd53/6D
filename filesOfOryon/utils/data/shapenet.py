...
def get_item_data(root: str, annots: dict, metadata: Tuple, img_id: int, cat_id: int=None) -> dict:
    ...
    scene_root = '/data/WDY/oryon-main/shapenet6d/shapenet6d'
    # scenes 是新的数据主目录
    scene_base = join(scene_root, 'scenes')
    img = np.asarray(Image.open(join(scene_base, 'rgb', f'{img_id:06d}.jpg')).convert('RGB'))
    mask = np.asarray(Image.open(join(scene_base, 'instance_segmap', f'{img_id:06d}.png')).convert('L'))
    depth = np.asarray(Image.open(join(scene_base, 'depth', f'{img_id:06d}.png')))
