...
class ObjectMatch:
    ...
    def _run_superglue(
        ...
        with open(temp_pairs, 'w') as f:
            ...
            if pose0 is not None and pose1 is not None:
                gt_pose = np.linalg.inv(np.linalg.inv(pose0) @ pose1)
                line = '{} {} 0 0 {} {} {}\n'.format(
                    os.path.abspath(image0_path),
                    os.path.abspath(image1_path),
                    ' '.join(map(str, intrinsic0[:3, :3].ravel().tolist())),
                    ' '.join(map(str, intrinsic1[:3, :3].ravel().tolist())),
                    ' '.join(map(str, gt_pose.ravel().tolist())),
                )
            else:
                # 没有位姿信息，使用单位矩阵
                identity = np.eye(4)
                line = '{} {} 0 0 {} {} {}\n'.format(
                    ...
                    ' '.join(map(str, intrinsic0[:3, :3].ravel().tolist())),
                    ' '.join(map(str, intrinsic1[:3, :3].ravel().tolist())),
                    ' '.join(map(str, identity.ravel().tolist())),
                )
            f.write(line)
        ...
        cmd = (
            f'{sys.executable} {sg_script} '
            f'--input_dir "/" '
            f'--input_pairs {temp_pairs} '
            f'--output_dir {self.match_cache_dir} '
            f'--superglue {self.superglue_model}'
        )
